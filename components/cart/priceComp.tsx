...
const PriceItems = () => {
  ...
  useEffect(() => {
    let price = cartArr.map((total: productDetails) => {
      ...
      //delivery
      let delivery = Math.floor(cartArr.map((del: productDetails) => del.shippingCost).reduce((a:number,b:number)=>a+b,0))
    ...
    let subTotal = Math.floor(
      ...
      return priceData;
       ...
       let myDiscount = discountArr.map((val:any)=>val.discountPrice).reduce((a:number,b:number)=>a+b,0)
  ...
  return (
      <div className="card p-3">
          {total===0? ( <div className="row">
              <div className="d-flex justify-content-center align-items-center"><BsMinecartLoaded size={50}/> Cart Empty</div>
      ...
      <div className="row d-flex align-items-center">
        ...
        <div className="row">
          <button className="btn btn-danger mb-2">Proceed to checkout</button>
          <button className="btn btn-outline-danger">Continue Shopping</button>
        </div>
      </div>
    </div>)}
